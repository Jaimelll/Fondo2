...
// filtraban por las becas existentes, así que un elemento recién creado en
// Catálogos nunca aparecía en el modal de Gestión de Servicios.

export async function getCondiciones() {
  try {
    const { rows } = await query(
      `select id, descripcion from condicion order by id asc`,
    );
...
export async function getInstitucionesBeca() {
  try {
    const { rows } = await query(
      `select id, descripcion from institucion order by descripcion asc`,
    );
    return rows.map((item: any) => ({ value: item.id, label: item.descripcion }));
  } catch {
    return [];
...
export async function getTiposEstudio() {
  try {
    const { rows } = await query(
      `select id, descripcion from tipo_estudio order by id asc`,
    );
    return rows.map((item: any) => ({ value: item.id, label: item.descripcion }));
  } catch {
    return [];
